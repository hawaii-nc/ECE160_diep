        ...
        if not game_over:
            ...
            if show_specialization_menu:
                ...
                if specialization_stage == 'root' or specialization_stage is None:
                    # show root choices
                    roots_text = "1-Dual  2-Twin  3-Heavy  4-Sniper"
                    root_text = font.render(roots_text, True, WHITE)
                    WIN.blit(root_text, (WIDTH//2 - 240, HEIGHT//2))
                elif specialization_stage == 'option' and current_tree is not None:
                    opt_text = f"{current_tree['label']} specialization: 1-{current_tree['options'][0]['label']} | 2-{current_tree['options'][1]['label']}"
