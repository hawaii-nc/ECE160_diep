...
class Tank:
    def __init__(self, x, y, color, is_player=False):
        ...
        self.shotgun_barrel = False       # true when shotgun upgrade is equipped
    ...
    def fire(self, aim_angle, owner_label, owner_id, shotgun=False):
        ...
        for mount in self.gun_mounts:
            ...
            if shotgun:
                ...
                # Spread pellets across ~60 degrees in front
                spread_angle = math.pi / 3.0  # 60 degrees total
                num_pellets = len(pellets)
                for i, p in enumerate(pellets):
                    # Distribute pellets evenly across the spread angle
                    angle_offset = (spread_angle / (num_pellets - 1)) * (i - (num_pellets - 1) / 2.0)
                    bullets.append(Bullet(self.x, self.y, emit_angle + angle_offset, p.speed, p.damage, p.radius, p.color, owner_label, owner_id))
            else:
                prof = mount.profile
    ...
    def draw(self, win, cam_x, cam_y, aim_angle):
        ...
        # Render barrels for each mount (rectangles scaled by bullet stats, or triangles if shotgun)
        for mount in self.gun_mounts:
            prof = mount.profile
            ...
            if self.shotgun_barrel:
                # Draw triangle with base facing away from tank
                # 60 degree angle means 30 degrees on each side
                half_angle = math.pi / 6.0  # 30 degrees
                base_width = width * 2.0  # Make base twice as wide
                pts_local = [
                    (0, 0),  # tip at tank body
                    (length, -base_width * math.tan(half_angle)),  # bottom right
                    (length, base_width * math.tan(half_angle)),   # top right
                ]
                
                # Rotate points by barrel angle
                ...
                # Draw rectangle for normal barrels
